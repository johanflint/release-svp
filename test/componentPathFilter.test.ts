...
describe("componentPathFilter", () => {
    ...
    describe("matchesComponentPath", () => {
        ...
        it("returns 'unknown' when no changed-file data is available", () => {
            expect(matchesComponentPath(undefined, undefined, "a", allComponentPaths)).toBe("unknown");
        });

        it("returns 'match' when a changed file belongs to the component", () => {
            expect(matchesComponentPath(["a/src/lib.rs"], false, "a", allComponentPaths)).toBe("match");
        });

        it("returns 'no-match' when no changed file belongs to the component", () => {
            expect(matchesComponentPath(["b/src/lib.rs"], false, "a", allComponentPaths)).toBe("no-match");
        });

        it("returns 'match' for the root component when a file belongs to no configured component", () => {
            expect(matchesComponentPath(["README.md"], false, "", allComponentPaths)).toBe("match");
        });

        it("returns 'no-match' for the root component when every file belongs to a more specific component", () => {
            expect(matchesComponentPath(["a/src/lib.rs"], false, "", allComponentPaths)).toBe("no-match");
        });

        it("returns 'match' and warns when the result is truncated and no match was observed", () => {
            vi.spyOn(logger, "warn");

            expect(matchesComponentPath(["b/src/lib.rs"], true, "a", allComponentPaths)).toBe("match");
            expect(logger.warn).toHaveBeenCalledWith("Changed files list was truncated, conservatively assuming it may affect component 'a'");
        });

        it("returns 'match' without warning when truncated but a match was already observed", () => {
            vi.spyOn(logger, "warn");

            expect(matchesComponentPath(["a/src/lib.rs"], true, "a", allComponentPaths)).toBe("match");
        ...
        it("warns when a changed file belongs to no configured component and no root component exists", () => {
            ...
            expect(matchesComponentPath(["README.md"], false, "a", allComponentPaths)).toBe("no-match");
        ...
        it("does not warn when a root component is configured", () => {
            ...
            expect(matchesComponentPath(["README.md"], false, "a", ["", "a", "b"])).toBe("no-match");
            expect(logger.warn).not.toHaveBeenCalled();
        });
        ...
        it("does not warn when at least one changed file belongs to a configured component", () => {
            ...
            expect(matchesComponentPath(["README.md", "b/src/lib.rs"], false, "a", allComponentPaths)).toBe("no-match");
            expect(logger.warn).not.toHaveBeenCalled();
        });
    });
});
